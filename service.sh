...
# 判断是否为超级省电模式
if [ "$PERFORMANCE" -eq "2" ]; then
  # 超级省电模式会限制性能释放
  module_log "当前模式: 超级省电（$PERFORMANCE）"
  # 限制所有后台应用、前台应用以及悬浮窗应用
  ...
  module_log "已设置 CPU 调度模式为 POWERSAVE 省电模式"
  # 启用调度自动分组
...
if [ "$PERFORMANCE" = "0" ]; then
  ...
  module_log "CPU 调度模式为 PERFORMANCE 性能模式"
  ...
  chmod 644 /sys/devices/system/cpu/cpu7/cpufreq/scaling_governor
  echo powersave > /sys/devices/system/cpu/cpu7/cpufreq/scaling_governor
  module_log "CPU 调度模式为 POWERSAVE 省电模式"
fi
